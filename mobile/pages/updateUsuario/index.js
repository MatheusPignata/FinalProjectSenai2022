...
export default function UpdateUsuario({ navigation }) {
    ...
    const atualizar = () => {
        ...
        fetch('http:/10.87.207.26:3000/'+cpf, {
            "method": "PUT",
            "headers": {
            ...
            .then(async data => {
                if (data.length > 0) {
                    ToastAndroid.show('Usuario atualizado', ToastAndroid.SHORT)
                } else {
                    ToastAndroid.show('Falha ao atualizar usuario', ToastAndroid.SHORT);
                }
            })
    }
